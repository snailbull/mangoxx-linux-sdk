...
static u8_t spiffs_fd_buf[512];
static u8_t spiffs_work_buf[1024];	//LOG_PAGE * 2
static u8_t spiffs_cache_buf[4096];

// low level read/write/erase
static s32_t flash_read (u32_t addr, u32_t size, u8_t *dst)
{
	memcpy (dst, flash_mem + addr, size);
	return SPIFFS_OK;
}

static s32_t flash_write (u32_t addr, u32_t size, u8_t *src)
{
	memcpy (flash_mem + addr, src, size);
	return SPIFFS_OK;
}

static s32_t flash_erase (u32_t addr, u32_t size)
{
	memset (flash_mem + addr, 0xff, size);
	return SPIFFS_OK;
}
...
int main (int argc, char *argv[])
{
	...
	// map flashmemory
	flash_mem = malloc(FS_IMAGE_SIZE);//mmap (0, FS_IMAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd_img, 0);
	if (!flash_mem)
	{
		die ("malloc");
	}
	if (create)
	{
		memset (flash_mem, 0xff, FS_IMAGE_SIZE);
	}
	else
	{
		// img to flash memory
	...
	spiffs_config cfg;
	cfg.phys_size = FS_IMAGE_SIZE;
	cfg.phys_addr = 0;
	cfg.phys_erase_block = LOG_BLOCK;
	...
	if (SPIFFS_mount (&fs, &cfg,
	                  ...
	                  spiffs_cache_buf, sizeof(spiffs_cache_buf), 0) != 0)
	{
		if (create)
		{
