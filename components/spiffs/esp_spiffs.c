...
static s32_t esp_spiffs_readwrite(u32_t addr, u32_t size, u8_t *p, int write)
{
    ...
    SpiFlashOpResult res = spi_flash_read((uint32)aligned_addr, (uint32 *) tmp_buf, aligned_size);
    ...
    if (!write) {
        os_memcpy(p, tmp_buf + (addr - aligned_addr), size);
        return SPIFFS_OK;
    }

    os_memcpy(tmp_buf + (addr - aligned_addr), p, size);

    res = spi_flash_write(aligned_addr, (uint32 *) tmp_buf, aligned_size);
...
s32_t esp_spiffs_init(struct esp_spiffs_config *config)
{
    ...
    spiffs_work_buf = (u8_t*)os_malloc(config->log_page_size * 2);
    if (spiffs_work_buf == NULL) {
        return -1;
    }
    ...
    spiffs_fd_buf = (u8_t*)os_malloc(config->fd_buf_size);
    if (spiffs_fd_buf == NULL) {
        os_free(spiffs_work_buf);
        return -1;
    }
    ...
    spiffs_cache_buf = (u8_t*)os_malloc(config->cache_buf_size);
    if (spiffs_cache_buf == NULL) {
        os_free(spiffs_work_buf);
        os_free(spiffs_fd_buf);
        return -1;
...
void myspiffs_init(void)
{
    ...
    config.fd_buf_size = FD_BUF_SIZE;
    config.cache_buf_size = CACHE_BUF_SIZE;
    if (esp_spiffs_init(&config) == -1)
    {
        os_printf("spiffs init error\n");
    }
    else
    {
        os_printf("ok\n");
...
void list (char *path)
{
    ...
    while (SPIFFS_readdir (&dir, &de))
    {
        ...
        os_memcpy (name, de.name, sizeof(de.name));
...
void cat (char *fname)
{
	...
	os_printf("\n");
    s32_t n;
    while ((n = SPIFFS_read (&fs, fh, buf, 1024)) > 0)
    {
        uart0_tx_buffer(buf, n);
    }
    SPIFFS_close (&fs, fh);
...
{
    spiffs_file fh = SPIFFS_open (&fs, fname, SPIFFS_CREAT | SPIFFS_TRUNC | SPIFFS_WRONLY, 0);
    if (fh < 0)
    {
        os_printf("spiffs_open err\n");
        return;
    }

    if (SPIFFS_write (&fs, fh, src, srclen) < 0)
        die ("spiffs_write");
