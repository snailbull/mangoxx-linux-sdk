...
// is dropped.
static ICACHE_FLASH_ATTR s32_t spiffs_gc_erase_block(
    spiffs *fs,
    spiffs_block_ix bix) {
  s32_t res;
...
ICACHE_FLASH_ATTR s32_t spiffs_gc_quick(
...
// cleansed and erased
ICACHE_FLASH_ATTR s32_t spiffs_gc_check(
    spiffs *fs,
    u32_t len) {
  s32_t res;
  s32_t free_pages =
...
ICACHE_FLASH_ATTR s32_t spiffs_gc_erase_page_stats(
    spiffs *fs,
    spiffs_block_ix bix) {
  s32_t res = SPIFFS_OK;
  int obj_lookup_page = 0;
...
ICACHE_FLASH_ATTR s32_t spiffs_gc_find_candidate(
    spiffs *fs,
...
//
ICACHE_FLASH_ATTR s32_t spiffs_gc_clean(spiffs *fs, spiffs_block_ix bix) {
  s32_t res = SPIFFS_OK;
  int entries_per_page = (SPIFFS_CFG_LOG_PAGE_SZ(fs) / sizeof(spiffs_obj_id));
  int cur_entry = 0;
  spiffs_obj_id *obj_lu_buf = (spiffs_obj_id *)fs->lu_work;
